...
def konversi_kunci():
    ...
    print("\nK:"
          ...
          + np.array2string(newsortkunci[2],
                            ...
                            # 'lamda x: x' digunakan untuk mempertahankan elemen-elemen string apa adanya
...
def encrypt(teks, kunci):
    ...
    # # ------------------------ PROSES ENKRIPSI ---------------------------
    # jika pengurutannya dari besar ke kecil
    # newenkripsi = enkripsi[:, enkripsi[0].argsort()[::-1]]
    newenkripsi = enkripsi[:, enkripsi[0].argsort()]

    # hitung jumlah baris terakhir
    hitung = Counter(newenkripsi[0])
    newenkripsi = newenkripsi[::-1]
    print(newenkripsi, "\n")

    print(hitung)
    print(hitung[newenkripsi[-1][2]])
    ...
    while i < len(newenkripsi[0]):
        ...
        if hitung[newenkripsi[-1][i]] == 1:
            for j in range(len(newenkripsi)-1):
                huruf = newenkripsi[j][i]
                hasil.append(huruf)
                # print("Huruf pada baris", j, "kolom", i, ":", huruf)
        else:
            for j in range(len(newenkripsi)-1):
                for k in range(i, hitung[newenkripsi[-1][i]] + i):
                    huruf = newenkripsi[j][k]
                    hasil.append(huruf)
            i += (hitung[newenkripsi[-1][i]])
        i += 1
