...
# contoh shift = 2
#  bits[shift:] = bits[2:]
# mengambil nilai bits selain 2 didepan
# bits[:shift] = bits[:2]
# mengambil nilai bits hanya nilai 2 dari depan
# return  00 + 01000100
...
# msg = text_to_binary(plain_text)
# key = text_to_hex(key_text)

# test1 = msg
# test2 = key

# paragraf = input("Masukkan paragraf untuk dienkripsi ")
# par_arr = modif_par(paragraf)
...
def encrypt_print(plain , key):
  ...
  print("Hasil enkrip dalam bentuk biner = " ,  hasil_encrypt)
  print("Hasil enkrip dalam bentuk hexa =  " , bin_to_hex(''.join(hasil_encrypt) , 64))
  print("Hasil enkrip dalam bentuk text = " , hex_to_text(bin_to_hex(''.join(hasil_encrypt) , 64)))
  return hasil_encrypt

def decrypt_print(chiper , key):
  hasil_decrypt = []
  hex_key = text_to_hex(key_text)
  for x in hasil_encrypt:
     print(hex_to_text( bin_to_hex(decrypt(x, hex_key), 64)))
     hasil_decrypt.append(hex_to_text( bin_to_hex(decrypt(x, hex_key), 64)))

  print(''.join(hasil_decrypt).rstrip('-'))
...
def main():
  while True:
        ...
        if choice == '1':
            ...
            plain_modif = modif_par(plain_text)
            ...
            cipher_modif = modif_par(cipher_text)
            decrypt_print(cipher_modif , key)
        elif choice == '3':
